...
while ($o = $res->fetch_object()) {
  ...
  $rolesRepeatB = false; # set to true if have > 1 role and not all diMes occur for all roles
  if ($numDimRoles > 1) {
    # have multiple roles so check to see if all diMes occur for all roles
    $numDiMeRoles = 0;
    foreach ($diMesAA as $rId => $diMesA) {
      if (!$numDiMeRoles) {
        $numDiMeRoles = count($diMesA);
        if ($numDiMeRoles != $numDimRoles)
          echo "\$numDiMeRoles $numDiMeRoles != dim roles count $numDimRoles for role $rId<br>";
      }else{
        if ($numDiMeRoles != count($diMesA)) {
          $rolesRepeatB = true;
          echo "\$numDiMeRoles $numDiMeRoles != count(\$diMesA) ". count($diMesA)." for role $rId<br>";
        }
      }
    }
  }
  $numEles = $rolesRepeatB ? $numDimRoles * $numRows : $numRows;
  if ($ShortB && $numEles>12)
  ...
  foreach ($diMesAA as $rId => $diMesA) { # [diMeId => [elId, level, bits, StdLabel]]]
    ...
    foreach ($diMesA as $diMeId => $diMeA) { # [elId, level, bits, StdLabel]
      # loop through diMees
      if ($ShortB && $numRows < $numEles && $ne>3) {
        if ($ne==4) {
