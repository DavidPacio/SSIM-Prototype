...
# Add some of the common XBRL Roles to get them in the desired Id order to match the TR_ constants
#                                    Role              UsedOn
# const TR_StdLabel          =  1; # standardLabel     label
# const TR_VerboseLabel      =  2; # verboseLabel      label
# const TR_TerseLabel        =  3; # terseLabel        label
# const TR_Documentation     =  4; # documentation     label
# const TR_NetLabel          =  5; # netLabel          label
# const TR_TotalLabel        =  6; # totalLabel        label
# const TR_NegLabel          =  7; # negatedLabel      label
# const TR_NegTerseLabel     =  8; # negatedTerseLabel label
# const TR_NegTotalLabel     =  9; # negatedTotalLabel label
# const TR_PeriodStartLabel  = 10; # periodStartLabel  label
# const TR_PeriodEndLabel    = 11; # periodEndLabel    label
# const TR_Reference         = 12; # reference         reference
# const TR_DisclosureRef     = 13; # disclosureRef     reference
# const TR_ExampleRef        = 14; # exampleRef        reference
# const TR_CommonPracticeRef = 15; # commonPracticeRef reference
# const TR_Link              = 16; # link              link,labelLink,referenceLink
...
$rolesA = [ # role       usedOn       definition
  ...
  ['link',              'link,labelLink,referenceLink', 'Link, Label Link, ReferenceLink'],
];
$RoleId=0;
...
# echo "<br><b>Updating Elements.StdLabel to speed Elements with Standard Label Fetches</b><br>";
...
function Element($nsId) {
  global $NodesA, $NumNodes, $NodeX, $XidsMI, $SchemaId; # , $NamesMI
  static $sElId=0;
  $node = $NodesA[$NodeX];
  $depth = $node['depth'];
  $set = "NsId='$nsId'";
  ...
  # const TSG_None        0; # NULL              /- Elements with these SGs are not stored in the Elements table
  # const TSG_LinkPart  = 4; # link:part         |
  # const TSG_Arc       = 5; # xl:arc            |
  # const TSG_Doc       = 6; # xl:documentation  |
  # const TSG_Extended  = 7; # xl:extended       |
  # const TSG_Locator   = 8; # xl:locator        |
  # const TSG_Resource  = 9; # xl:resource       |
  # const TSG_Simple    =10; # xl:simple         |
  # const TSG_Tuple     =11; # xbrli:tuple       |
...
function RoleType() {
  ...
  if (!strpos($roleURI, $idS)) {
    ...
    # }
    # echo "roleType at node $NodeX of the Schema has an id if '$idS' which differs from the role '$role' from the roleURI '$roleURI'. Using $usingRole<br>";
    # if ($changedB)
    #   # change $rolURI so that UpdateRole() will get $usingRole
    #   $roleURI = str_replace($role, $usingRole, $roleURI);
    echo "roleType at node $NodeX of the Schema has an id if '$idS' which differs from the role '$role' from the roleURI '$roleURI'. Using $role<br>";
  }
...
function XLink($typeN) {
  ...
  $LocLabelToIdA = []; # [label => [[id, TAFTT_Element | TAFTT_Label | TAFTT_Reference | TAFTT_Role]]] can have multiple entries for the same label. idS = locator idS, id == resource (Resources table) Id
  $node = $NodesA[$NodeX];
   ...
   #$tag = StripPrefix($NodesA[$NodeX]['tag']); # strip leading link: or label: in the gen:link case
    ...
    switch ($tag) {
      case 'link:loc':
        ...
        $LocLabelToIdA[$label][] = $idA; # [label => [[id, TAFTT_Element | TAFTT_Label | TAFTT_Reference | TAFTT_Role]]]
        break;
...
function Arc($typeN, $pRoleId) {
  global $DB, $NodesA, $NodeX, $XidsMI, $LocLabelToIdA; # $XidsMI [xidS  => ElId], $LocLabelToIdA [label => [[id, TAFTT_Element | TAFTT_Label | TAFTT_Reference | TAFTT_Role]]] can have multiple entries for the same label
  $node = $NodesA[$NodeX];
  ...
  $baseSet = $set;
  # Updating Resources ElId didn't work for references because of same reference used for multiple elements
  # Did work for just labels but having just that is a bit pintless and dangerous as some taxonomy might use the same label for multiple element. so Resources.ElId was dropped 2018.10.25.
  # Die - Resources Id 5608 has its ElId set to 924 already when trying to set it to 915
  # foreach ($fromIdsA as $j => $idA) {
  #   $fromId    = $idA[0];
  #   $fromTypeN = $idA[1];
  #   if (!($fromTypeN == TAFTT_Element || $fromTypeN == TAFTT_Role))
  #     DieNow("Arc fromTypeN $fromTypeN not Element or Role");
  #   $fromSet = $baseSet.",FromId=$fromId,FromTypeN=$fromTypeN,ToId="; # FromTypeN = TAFTT_Element | TAFTT_Role
  #   foreach ($toIdsA as $j => $idA) {
  #     $toId    = $idA[0];
  #     $toTypeN = $idA[1];
  #     InsertFromLinkbase('Arcs', $fromSet.$toId.",ToTypeN=$toTypeN"); # ToTypeN = TAFTT_Element | TAFTT_Label | TAFTT_Reference | TAFTT_Role
  #    #if ($fromTypeN == TAFTT_Element && ($toTypeN == TAFTT_Label || $toTypeN == TAFTT_Reference)) {
  #     if ($fromTypeN == TAFTT_Element && $toTypeN == TAFTT_Label) {
  #       // Resource so update ElId in the Resources table
  #       $elId = $DB->OneQuery("Select ElId From Resources where Id=$toId");
  #       if ($elId)
  #         DieNow("Resources Id $toId has its ElId set to $elId already when trying to set it to $fromId");
  #       $DB->StQuery("Update Resources Set ElId=$fromId Where Id=$toId");
  #     }
  #   }
  # }
  # Insert the arcs without updating Resources.ElId
  foreach ($fromIdsA as $j => $idA) {
    $fromSet = $baseSet.",FromId={$idA[0]},FromTypeN={$idA[1]},ToId="; # FromTypeN = TAFTT_Element | TAFTT_Role
    foreach ($toIdsA as $j => $idA)
      InsertFromLinkbase('Arcs', $fromSet.$idA[0].",ToTypeN={$idA[1]}"); # ToTypeN = TAFTT_Element | TAFTT_Label | TAFTT_Reference | TAFTT_Role
  }
} # End Arc()
...
function Label($node) {
  ...
  $id = InsertFromLinkbase('Resources', $set);
    $LocLabelToIdA[$label][] = [$id, TAFTT_Label]; # [label => [[id, TAFTT_Element | TAFTT_Label | TAFTT_Reference | TAFTT_Role]]] can have multiple entries for the same label
} // End Label()
