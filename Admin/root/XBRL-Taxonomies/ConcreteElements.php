<?php /* \Pacio\Development\SSIM Proto\Admin\root\XBRL-Taxonomies\ConcreteElements.php

Lists Taxonomy Concrete Elements
...
#$maxLevel = $maxId = $maxPres = 0;
 $res = $DB->ResQuery('Select E.Id,T.Text,Hypercubes,E.TetN,PeriodN,SignN,TesgN From Elements E Join Text T on T.Id=E.StdLabelTxtId Where TesgN In (1,2) and abstract is null');
#$res = $DB->ResQuery('Select E.Id,T.Text,Hypercubes,E.TetN,PeriodN,SignN From Elements E Join Text T on T.Id=E.StdLabelTxtId Where TesgN=1 and abstract is null');
#$res = $DB->ResQuery('Select E.Id,T.Text,Hypercubes,E.TetN,PeriodN,SignN From Elements E Join Text T on T.Id=E.StdLabelTxtId Where TesgN=1 and abstract is null and E.Id in (4155, 4290, 85,5167,5266,5267,511,4769,557, 1125)'); # 511,62, 5266, 5267, 5167, 5169, 5189, 85, 1125)');
$tot = $res->num_rows;
while ($o = $res->fetch_object()) {
  $id     = (int)$o->Id;
  $label  = $o->Text;
  $type   = ElementTypeToStr($o->TetN);
  $period = PeriodTypeToStr($o->PeriodN);
  $sign   = $o->TetN == TETN_Money ? SignToStr($o->SignN) : '';
  ...
  foreach ($infoA as $pRoleId => $infoA)
    if ($pRoleId < TRId_FirstHypercubeRoleId)
  ...
  if ($nDef) { # TesgN=1 Concrete Item. Never a Tuple
    ++$items;
    foreach ($defInfoA as $pRoleId => $infoA) {
      ...
      $kids     = $infoA[2];
      if ($pRoleId < TRId_FirstHypercubeRoleId)
        die("Die with pRoleId=$pRoleId < TRId_FirstHypercubeRoleId");
      else
        $hyId = $pRoleId - TRId_FirstHypercubeRoleId + 1;
      if (!$n) {
        $numTx = 0;
    ...
    # No definition tree entries =  all the TesgN=2 Concrete Tuples but also 10 others which have no hypercubes and no def tree:
  /*if ($o->TesgN != TESGN_Tuple) echo "$id no def tree but not tuple<br>";
...
Tuples=$tuples of which $tuplesInBrosTupleLists are in use for the $brosWithTuple Bros with tuple members<br>
...
function Kids($id, $pRoleId) {
  global $DB;
  $tltN = $pRoleId < TRId_FirstHypercubeRoleId ? TLTN_Presentation : TLTN_Definition;
  $res = $DB->ResQuery("Select ToId From Arcs Where FromId=$id And PRoleId=$pRoleId And TltN=$tltN");
  if ($res->num_rows) {
    $kids = '';
    while ($o = $res->fetch_object()) {
      $toId = (int)$o->ToId;
      $kids .= ",$toId" . Kids2($toId, $pRoleId, $tltN);
    }
    $res->free();
    $kids = substr($kids, 1);
  }else{
...
function Kids2($id, $pRoleId, $tltN) {
  global $DB;
  $res = $DB->ResQuery("Select ToId From Arcs Where FromId=$id And PRoleId=$pRoleId And TltN=$tltN");
  if ($res->num_rows) {
    $kids = '(';
